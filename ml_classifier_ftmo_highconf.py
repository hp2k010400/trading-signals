...
TRADING RULE (built on the model's output):
  - P(price up in 5 days) > 0.65 -> long. < 0.35 -> short. Else: no
    trade. (Raised from 0.55/0.45 in the first version -- only trade
    the model's most confident calls, given the unfiltered version
    came back PF 1.103, real but below target, with a weak most-recent
    window.)
...
Run in Codespace: python -u ml_classifier_ftmo_highconf.py
"""
import pandas as pd
import numpy as np
import os, warnings

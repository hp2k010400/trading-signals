"""
fair_price_montecarlo_stress.py

Deliberately handicapped re-run of fair_price_montecarlo.py's Monte Carlo,
specifically to stress-test the suspiciously clean "100% pass rate"
result before trusting it with a real account. Two changes:

  1. COST STRESS: re-run at 1x (baseline), 2x, and 3x the modeled
     transaction cost. If a real edge is genuine, it should degrade
     gracefully under worse costs, not require the exact modeled cost to
     survive.

  2. BLOCK BOOTSTRAP instead of single-day resampling: draws BLOCKS of 5
     consecutive real calendar days at a time, not one independent day.
     If bad days actually cluster in reality (a real regime shift, a
     volatile week), single-day iid resampling would wash that out by
     always mixing a bad day back in with average days immediately after.
     Block bootstrap preserves any real day-to-day autocorrelation in
     performance instead of assuming every day is independent.

Only tests risk levels 0.02%, 0.05%, 0.08%, 0.10% -- the ones actually in
consideration -- to keep this fast.

Run in Codespace: python -u fair_price_montecarlo_stress.py
"""
import pandas as pd
import numpy as np
import os, warnings
...
RISK_LEVELS = [0.02, 0.05, 0.08, 0.10]
...
by_day_gross = [None] * len(days_sorted)
by_day_cost = [None] * len(days_sorted)
...
def simulate_one(rng, rpt, cost_mult, block_days):
    equity = START_BAL
    peak = START_BAL
    day_i = 0
...
print(f'Stress sweep: risk x cost-multiplier, {BLOCK_DAYS}-day block bootstrap, {MC_RUNS:,} runs each')
print(f'{"="*100}')
print(f'  {"Risk/trade":>10}  {"Cost x":>7}  {"PASSED":>8}  {"FAILED_DAILY":>13}  {"FAILED_TOTAL":>13}  '
      f'{"Median days":>12}')
print(f'  {"-"*98}')
...
for risk_pct in RISK_LEVELS:
    ...
    for cost_mult in COST_MULTIPLIERS:
        ...
        results = [simulate_one(rng, rpt, cost_mult, BLOCK_DAYS) for _ in range(MC_RUNS)]

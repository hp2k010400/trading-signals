"""
alpha03_strategy_usdindex_leadlag_ftmo.py

Converts alpha03's descriptive finding into a real, tradeable, costed
strategy: USDINDEX -> equity indices lead-lag was directionally
consistent across ALL 11 equity indices tested (strong-USD day
predicts a weaker next-day index return, weak-USD day predicts a
stronger one) -- economically grounded in dollar-strength/funding-
stress spillover into risk assets, not a coincidence found by
searching many combinations.

MECHANISM (no lookahead -- the quintile thresholds in the earlier
descriptive script used the FULL SAMPLE, which would be lookahead if
traded directly; this version uses a trailing rolling window only):
  1. Each day, compute USDINDEX's daily return.
  2. Compute the ROLLING (trailing LOOKBACK_DAYS, no future data)
     80th and 20th percentile of USDINDEX's own daily returns.
  3. If today's USDINDEX return >= rolling 80th percentile: SHORT every
     equity index tomorrow (strong-USD day -> expect weaker indices).
  4. If today's USDINDEX return <= rolling 20th percentile: LONG every
     equity index tomorrow.
  5. Entry at tomorrow's open (first tradeable price after the signal
     is fully known). Exit at tomorrow's close (matching the 1-day
     lead-lag window the descriptive test measured). ATR-based stop
     for real risk control (the descriptive test had none), target =
     stop x RR.
...
split as everything else -- full history vs a held-out recent window,
selection never touches the holdout.

Run in Codespace: python -u alpha03_strategy_usdindex_leadlag_ftmo.py
"""
import pandas as pd
import numpy as np
...
LEADER_FILE = 'USDINDEX_M1_ftmo.csv'
INDEX_FILES = {
...
def load_leader_signal():
    ...
    # rolling, causal quantile thresholds -- no future data
    ...
    daily['ret1_lagged'] = daily['ret1']   # today's return, known at today's close
    return daily.dropna(subset=['q80', 'q20'])


def load_follower_h1(symbol):
    fn = INDEX_FILES[symbol]
...
print('Loading USDINDEX leader signal...')
...
for symbol in INDEX_FILES:
    ...
    for signal_date, row in leader.iterrows():
        ...
        # entry: tomorrow's first H1 bar open (first tradeable price after signal known)
        ...
        # exit: end of that same tradeable day (~24h later), matching the 1-day lead-lag window
...
print(f'Loaded {len(loaded)} follower indices: {loaded}')

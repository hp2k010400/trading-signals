...
User noticed a couple of London-session losers this morning and asked
whether the system should just drop London entirely. Two trades isn't
meaningful evidence on its own, but the underlying question is real
and testable: does removing London help, hurt, or barely matter?

Context already established tonight: NY fires far more reliably
(86% of days have a trade vs London's 54%), and London's trade count
leans heavily on DAX specifically (the only instrument that's
actually balanced between sessions -- everything else, especially the
3 US indices, is 2-7x more active in NY). So it's plausible NY-only
could be neutral or even better on a risk-adjusted basis -- or it
could just mean fewer total trades with no real quality gain, since
DAX itself performs fine in London. Worth checking properly.

Compares BOTH (the current live system) vs NY-ONLY: overall stats,
plus a walk-forward check on the NY-only variant so a "looks better
in aggregate" result can't hide a few bad windows.

Run in Codespace: python -u ny_only_check.py
"""
import pandas as pd
import numpy as np
import os, warnings
...
SESSIONS_BOTH = {'LONDON': 8, 'NY': 13}
SESSIONS_NY_ONLY = {'NY': 13}
...
for label, sessions in [('BOTH (current live system)', SESSIONS_BOTH), ('NY-ONLY (London removed)', SESSIONS_NY_ONLY)]:
    all_trades = []
    ...
    for i in range(0, len(all_periods), WALK_FORWARD_MONTHS):
        ...
        if tot2 < 0:
            n_losing += 1
    print(f'  Walk-forward: {n_losing}/{n_total} losing 6-month windows\n')

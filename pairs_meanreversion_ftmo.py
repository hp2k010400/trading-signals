"""
pairs_meanreversion_ftmo.py

Retest of pairs_meanreversion.py, now on FTMO's own broker-native
data instead of OANDA -- the first strategy built from the start with
tonight's lesson in mind: this mechanism (rolling 20-day relative
return spread between two instruments) is structurally coarse and
NOT dependent on any single candle's exact wick, unlike the
displacement strategy that just failed to transfer from OANDA to
FTMO. Includes the confirmed UTC+3 broker offset correction.

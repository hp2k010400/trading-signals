"""
edge19_strategy_validation.py

Gates 2-7 for Edge #1 (E19): CFTC commercial net positioning -> S&P 500
forward returns, converted to the simplest executable strategy.

PRESERVES THE PRE-REGISTRATION IN EDGE19_HYPOTHESIS.MD EXACTLY:
  - 156-week z-score lookback (not re-tuned)
  - net_comm_frac signal definition (not re-tuned)
  - 3-day publish-lag no-lookahead rule (not re-tuned)
  - 1/2/4-week horizons, ALL THREE reported, none selected as "the best"
  - S&P 500 as primary instrument; NAS100/US30/US2000 as an unmodified
    generalization check (same signal construction applied to their own
    COT + price history, no per-instrument re-tuning)

Strategy is deliberately the simplest possible expression of the
pre-registered signal: direction = sign(z) (long when commercial
positioning is less-short-than-usual, short when more-short-than-usual),
no threshold/deadzone (that would be a new, un-registered parameter),
vol-scaled position sizing (RISK_PCT convention used throughout this
research programme), one trade per weekly COT report per horizon.

Gate 7 parameter-stability sweep varies the z-score lookback around the
pre-registered 156 weeks and reports ALL of them -- it does NOT adopt
whichever looks best. The canonical result is always the pre-registered
156-week version regardless of what the sweep shows.

Runs on real data directly (CFTC + Yahoo Finance are both reachable from
this environment).
"""
...
# ============================================================
# PRE-REGISTERED CONSTANTS -- DO NOT MODIFY BASED ON RESULTS
# ============================================================
...
# ============================================================
# Strategy-construction conventions (standard throughout this
# research programme, not tuned to this edge specifically)
# ============================================================
RISK_PCT = 0.30
...
def compute_signal(cot, zwin):
    ...
    cot['z'] = (cot['net_comm_frac'] - cot['net_comm_frac'].rolling(zwin).mean()) / \
               cot['net_comm_frac'].rolling(zwin).std()
...
def build_trades(cot, px, horizon_weeks, symbol=PRIMARY_SYMBOL):
    ...
    for _, r in cot.iterrows():
        ...
        period_vol = px_vol[entry_pos] * np.sqrt(5 * horizon_weeks)  # ~5 trading days/week
...
# ============================================================
# GATE 2-3: build trades, apply costs, for the primary instrument (SP500)
# ============================================================
print(f'{"#"*100}\n  GATE 2-3: SP500 strategy construction + realistic costs\n{"#"*100}')
print('Fetching SP500 COT + price data...')
...
for h in HORIZONS_WEEKS:
    ...
    print(f'\n--- Horizon: {h}-week hold, N={len(t)} trades, {len(t)/((t["entry_date"].max()-t["entry_date"].min()).days/30.4):.1f} trades/month ---' if len(t) else f'\n--- Horizon {h}w: no trades ---')
...
# ============================================================
# GATE 4 (part 1): BY YEAR -- primary horizon set = all three, base cost
# ============================================================
print(f'\n{"#"*100}\n  GATE 4: BY YEAR (net, base cost) -- checking for single-period dependence\n{"#"*100}')
for h in HORIZONS_WEEKS:
...
# ============================================================
# GATE 4 (part 2): DISCOVERY / VALIDATION / FINAL-OOS
# ============================================================
...
for h in HORIZONS_WEEKS:
    ...
    print(f'\n  --- {h}-week horizon ---  (Discovery < {disc_end.date()} <= Validation < {val_end.date()} <= Final OOS)')
...
# ============================================================
# GATE 6: PERMUTATION TEST (circular shift, preserves each series' own
# autocorrelation, destroys only the specific temporal pairing)
# ============================================================
...
for h in HORIZONS_WEEKS:
    ...
    z_vals = t['z'].values
    raw_signed_no_direction = (t['r_gross'].values * t['direction'].values)  # undo direction to get raw log ret / vol, i.e. reconstruct unsigned base
    # Simplest correct approach: recompute r_net under permuted z (permuted direction & cost basis unchanged since cost is direction-independent)
    cost_component = t['cost_r_unit'].values * BASE_COST_MULT
    raw_ret_over_vol = t['r_gross'].values * t['direction'].values  # = raw_log_ret / period_vol (direction-free)
...
# ============================================================
# GATE 7: PARAMETER STABILITY -- sweep z-score lookback, report all,
# do NOT adopt the best one. Canonical = 156 weeks regardless.
# ============================================================
...
# ============================================================
# GATE 9 (generalization): NAS100, US30, US2000 -- same construction, unmodified
# ============================================================

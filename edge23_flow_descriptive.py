"""
edge23_flow_descriptive.py

Gate 1 descriptive test for Edge #5 (E23): does week-over-week CHANGE
in CFTC speculator net positioning (flow, not level) predict forward
S&P 500 returns? Pre-registered in EDGE23_HYPOTHESIS.md.

Predicted direction: POSITIVE (fresh net buying flow -> continuation).
"""
...
print('Fetching CFTC COT data + S&P 500 prices...')
cot = fetch_cot()
px = fetch_sp500()
...
cot['flow'] = cot['net_noncomm_frac'].diff()
cot['z'] = (cot['flow'] - cot['flow'].rolling(ZSCORE_WINDOW_WEEKS).mean()) / cot['flow'].rolling(ZSCORE_WINDOW_WEEKS).std()
...
print(f'\n{"="*90}\n  FULL HISTORY: speculator flow z-score -> forward S&P 500 return\n{"="*90}')

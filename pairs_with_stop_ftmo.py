"""
pairs_with_stop_ftmo.py

Pairs mean-reversion had no hard stop -- a trade could only exit via
the spread reverting (|z|<0.5) or a 20-day timeout, meaning a spread
that kept diverging just rode the full 20 days accumulating loss.
That's likely the main driver of the worst months (-£1,623, -2.32%)
and the poor risk-sweep economics. Adds a real stop: exit early if
the z-score extends even further against the position, past
STOP_Z, instead of waiting out the full 20 days.

Same locked walk-forward + monthly P&L discipline as
pairs_walkforward_monthly.py, same 7 surviving pairs, same real
spread costs, confirmed UTC+3 offset.

Run in Codespace: python -u pairs_with_stop_ftmo.py
"""
import pandas as pd
import numpy as np
import os, warnings
...
ENTRY_Z = 2.0
EXIT_Z = 0.5
STOP_Z = 3.5   # NEW: exit early if the spread diverges even further against us
MAX_HOLD_DAYS = 20
...
# NAS100/SP500 and NAS100/US30 dropped -- consistent losers in both
# IS and holdout in the first-look test, likely too tightly
# correlated with NAS100 for a real mean-reverting spread to exist
PAIRS = [
...
def simulate_pair(sym_a, sym_b):
    ...
    for i in range(ZSCORE_WINDOW + LOOKBACK_DAYS, n - 1):
        ...
        exited = False
        # stop-loss: if z has moved further against the position's original
        # direction (i.e. spread kept diverging instead of reverting), cut
        # it rather than riding the full 20-day hold
        stretched_further = (state['direction'] == -1 and z > STOP_Z) or \
                             (state['direction'] == 1 and z < -STOP_Z)
        if abs(z) < EXIT_Z:
            exited = True
        elif stretched_further:
            exited = True
...
print(f'\nTotal trades (7 surviving pairs): {len(df)}')
if len(df) < 80:

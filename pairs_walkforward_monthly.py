"""
pairs_walkforward_monthly.py

Walk-forward validation + real monthly P&L for the pairs mean-reversion
strategy, following its promising first IS/OOS result on FTMO data
(IS PF 1.12, HOLDOUT PF 2.24). Same discipline NEWGOATv1 got before
going live -- and given how that turned out despite passing, no
shortcuts here either.

Drops NAS100/SP500 and NAS100/US30 -- the two pairs that were
flat-to-losing in BOTH in-sample and holdout, a consistent rejection
(likely too tightly correlated with NAS100 for a genuine mean-
reverting spread to exist). Keeps the 7 pairs that looked coherent.

Reports:
  1. Non-overlapping 6-month walk-forward windows across the full
     history (not just one IS/OOS split).
  2. Real month-by-month P&L, each month simulated fresh from £70,000
     at 0.30% risk per trade (illustrative risk level, matching the
     convention used all night -- not yet separately calibrated for
     this strategy specifically).

Run in Codespace: python -u pairs_walkforward_monthly.py
"""
import pandas as pd
import numpy as np
import os, warnings

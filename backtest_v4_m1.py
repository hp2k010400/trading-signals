"""
backtest_v4_m1.py  -  V4 System Backtest with M1 Exit Simulation
=================================================================
Same signal logic as backtest_v4.py (H1 ORB/LC triggers) but
trade exits are simulated on M1 bars for accurate trail stop tracking.

Requires M1 files from download_dukascopy.py:
  EURUSD_M1_duka.csv, GBPUSD_M1_duka.csv, etc.

Run:  python backtest_v4_m1.py
"""
import pandas as pd
import numpy as np
...
M1_FILES = {
    'EURUSD': 'EURUSD_M1_duka.csv',
    'GBPUSD': 'GBPUSD_M1_duka.csv',
    'DAX':    'GER40_M1_duka.csv',
    'NAS100': 'US100_M1_duka.csv',
    'SP500':  'US500_M1_duka.csv',
    'UK100':  'UK100_M1_duka.csv',
    'GOLD':   'XAUUSD_M1_duka.csv',
}
...
def load_h1(key):
    ...
    fname = H1_FILES.get(key)
    if not fname or not os.path.exists(fname): _h1_cache[key] = None; return None
    df = pd.read_csv(fname)
    df['time'] = pd.to_datetime(df['time'], unit='s', utc=True)
    df = df.set_index('time').sort_index()
    for c in ['open','high','low','close']: df[c] = pd.to_numeric(df[c], errors='coerce')
    result = df.dropna() if len(df) > 200 else None
    _h1_cache[key] = result; return result

def load_m1(key):
    if key in _m1_cache: return _m1_cache[key]
    fname = M1_FILES.get(key)
    if not fname or not os.path.exists(fname): _m1_cache[key] = None; return None
    print(f'  Loading M1 data: {fname}...', end=' ', flush=True)
    df = pd.read_csv(fname)
    df['time'] = pd.to_datetime(df['time'], unit='s', utc=True)
    df = df.set_index('time').sort_index()
    for c in ['open','high','low','close']: df[c] = pd.to_numeric(df[c], errors='coerce')
    result = df.dropna() if len(df) > 200 else None
    _m1_cache[key] = result
    print(f'{len(result):,} bars' if result is not None else 'NOT FOUND')
    return result
...
def sim_m1(m1_df, entry_time, direction, entry, sl, trail, max_hours=80):
    """Simulate trade exit on M1 bars. More accurate trail stop tracking."""
    sl_d = abs(entry - sl)
    if sl_d <= 0: return -1.0
    tr = sl_d * trail
    cs = sl; bst = entry; be = False

    # Find entry bar in M1 data
    ep = m1_df.index.searchsorted(entry_time)
    if ep >= len(m1_df): return -1.0

    max_bars_m1 = max_hours * 60

    for i in range(ep, min(ep + max_bars_m1, len(m1_df))):
        b = m1_df.iloc[i]
        if direction == 1:
    ...
    last_idx = min(ep + max_bars_m1, len(m1_df) - 1)
    lp = m1_df.iloc[last_idx]['close']
    return ((lp - entry) if direction == 1 else (entry - lp)) / sl_d

def sim_h1_fallback(h1_df, ep, direction, entry, sl, trail, max_bars=80):
    """H1 fallback when M1 data not available."""
    sl_d = abs(entry - sl)
    if sl_d <= 0: return -1.0
    tr = sl_d * trail; cs = sl; bst = entry; be = False
    for _, b in h1_df.iloc[ep+1: ep+1+max_bars].iterrows():
        if direction == 1:
            if b['low'] <= cs: return (cs - entry) / sl_d
            bst = max(bst, b['high'])
            if not be and bst >= entry + sl_d: be = True; cs = entry
            ...
            if be:
                ns = bst + tr
                if ns < cs: cs = ns
    lp = h1_df.iloc[min(ep + max_bars, len(h1_df)-1)]['close']
    return ((lp - entry) if direction == 1 else (entry - lp)) / sl_d
...
def run_orb(key, tag, ref_h, es, ee, rmin, rmax, risk, trail, cost, skip_dow=frozenset()):
    ...
    for date in sorted(set(h1.index.normalize().date)):
        ...
        for j in range(len(edf)):
            b = edf.iloc[j]
            entry_time = edf.index[j]
            p = ipos_h1(h1, entry_time)
            if p < 0: continue
            if b['high'] > rhi:
                if m1 is not None:
                    r = sim_m1(m1, entry_time, 1, rhi, rlo, trail)
                else:
                    r = sim_h1_fallback(h1, p, 1, rhi, rlo, trail)
                trades.append({'pnl':_pnl(r,risk,cost),'date':str(date),'tag':tag,'r':r}); break
            if b['low'] < rlo:
                if m1 is not None:
                    r = sim_m1(m1, entry_time, -1, rlo, rhi, trail)
                else:
                    r = sim_h1_fallback(h1, p, -1, rlo, rhi, trail)
                trades.append({'pnl':_pnl(r,risk,cost),'date':str(date),'tag':tag,'r':r}); break
    return trades
...
def run_lc(key, tag, min_move, risk, trail, cost):
    ...
    for date in sorted(set(h1.index.normalize().date)):
        ...
        p  = ipos_h1(h1, day + pd.Timedelta(hours=16))
        if p < 0: continue
        entry_time = day + pd.Timedelta(hours=16)
        entry = h1.iloc[p]['open']
        if move > 0: sl = dh + buf; d = -1
        else:        sl = dl - buf; d =  1
        if d == -1 and sl <= entry: continue
        if d ==  1 and sl >= entry: continue
        if m1 is not None:
            r = sim_m1(m1, entry_time, d, entry, sl, trail)
        else:
            r = sim_h1_fallback(h1, p, d, entry, sl, trail)
...
def print_yearly(all_trades):
    W = 80
    print('\n' + '='*W)
    print('  1. YEAR-BY-YEAR BREAKDOWN  (M1 exit simulation)')
    print('='*W)
    ...
    print('  ' + 'Year  ' + ''.join('{:>10}'.format(st) for st in strats))
    print('  ' + '-'*86)
    for yr in sorted(by_year):
        row = '  ' + yr + '  '
        for stg in strats:
...
def print_walkforward(all_trades):
    W = 80
    print('\n' + '='*W)
    print('  2. WALK-FORWARD  (3-year IS / 1-year OOS, 5 folds)')
    print('='*W)
    ...
    if oos_pfs:
        print('  ' + '-'*66)
        print('  OOS PF: {:.2f} -- {:.2f}  |  Mean: {:.2f}'.format(
            min(oos_pfs), max(oos_pfs), np.mean(oos_pfs)))
        verdict = ('All >= 1.3  consistent edge confirmed' if min(oos_pfs) >= 1.3 else
                   'All profitable, some variance' if min(oos_pfs) >= 1.0 else
                   'WEAK OOS fold -- investigate')
...
def run_monte_carlo(all_trades):
    ...
    for _ in range(MC_RUNS):
        ...
        passed = failed = False; peak_dd = 0.0
        for dp in seq:
            day_start = eq; eq += dp
    ...
    W = 80
    print('\n' + '='*W)
    print('  3. MONTE CARLO -- FTMO PHASE 1  ({:,} simulations)'.format(MC_RUNS))
    print('  Target +{:,.0f}  |  Daily -{:,.0f}  |  Total -{:,.0f}'.format(
        target, daily_limit, total_limit))
    print('='*W)
    print('\n  Passed:                {:>5,}  ({:.1f}%)'.format(passes, pass_rate))
    ...
    print('  Still open at end:     {:>5,}  ({:.1f}%)'.format(still_open, still_open/MC_RUNS*100))
    print('\n  Median peak DD:        {:>8,.0f}'.format(np.median(peak_dds)))
    ...
    elif pass_rate >= 60: verdict = 'Moderate -- review params'
    else:                 verdict = 'Weak -- do not deploy'
    print('\n  -- PASS RATE: {:.1f}%  ->  {} --\n'.format(pass_rate, verdict))

if __name__ == '__main__':
    W = 80
    m1_available = [k for k in M1_FILES if os.path.exists(M1_FILES[k])]
    h1_only = [k for k in H1_FILES if k not in m1_available]

    print('\n' + '='*W)
    print('  V4 SYSTEM -- M1 BACKTEST  (70,000 GBP FTMO)')
    print('  M1 exit simulation: {}  |  H1 fallback: {}'.format(
        ', '.join(m1_available) if m1_available else 'NONE',
        ', '.join(h1_only) if h1_only else 'none'))
    print('='*W)

    if not m1_available:
        print('\n  No M1 data found. Run download_dukascopy.py first.')
        exit(1)

    print('\n  Loading M1 data...')
    for k in M1_FILES: load_m1(k)

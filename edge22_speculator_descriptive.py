"""
edge22_speculator_descriptive.py

Gate 1 descriptive test for Edge #4 (E22): does CFTC non-commercial
(speculator) net positioning EXTREME predict a reversal in forward
S&P 500 returns? Pre-registered in EDGE22_HYPOTHESIS.md BEFORE this
script was run.

Predicted direction (locked in advance): NEGATIVE correlation
(extreme long speculative positioning -> below-average forward return).

DESCRIPTIVE ONLY. Real data (CFTC Socrata API + Yahoo Finance), both
...
print('Fetching CFTC COT data (S&P 500 Consolidated, non-commercial)...')
cot = fetch_cot()
...
print(f'\n{"="*90}\n  FULL HISTORY: net_noncomm z-score -> forward S&P 500 return\n{"="*90}')
print('  (predicted: NEGATIVE correlation -- extreme long speculative positioning -> reversal)')
for h in HORIZONS_WEEKS:
